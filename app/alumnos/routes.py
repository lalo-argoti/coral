...
def almns_calificaciones():
    ...
    return render_template('alumnos/calificaciones.html', resultados=resultados, username=session.get('username'), titulo="estudiantes", menu_data =  ast.literal_eval(session['menu']))

@alumnos.route(p+'/acudientes')
def almns_acudientes():
    resultados= DB(query=f'SELECT * FROM occb_acudientes;', username="").run_query()
    return render_template('core/tabla2.html', datos=resultados,encabezados=["identidad","Nombres","Ocupación","Dirección","Celular","Correo"], username=session.get('username'), titulo="acudientes", menu_data =  ast.literal_eval(session['menu']))

@alumnos.route(p+'/promocion')
def almns_promocion():
    resultados = promocion()
    DB(query="-- desde promocion", username="").run_query()
    return render_template('alumnos/promocion.html', resultados=resultados , username=session.get('username'),menu_data =  ast.literal_eval(session['menu']))
...
def r_portal():
   # Aquí podrías obtener datos de la base de datos
    ...
    return render_template('core/portal.html', menu=itemsMenu, titulo= "Alumnos", username=session.get('username'),menu_data =  ast.literal_eval(session['menu']))

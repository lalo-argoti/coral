...
def usuario():
    ...
    if result:
        ...
        saludo = ""
    return render_template('usuario.html', saludo=saludo, empresa=empresa)

def f_vector2json(vector, corto):
    # Convierte el vector de tuplas a una lista de diccionarios
    noticias_json = []
    for noticia in vector:
        # Cada tupla se convierte en un diccionario con claves apropiadas
        if corto :
            noticiaC= noticia[2][0:400]+"..."
        else:
            noticiaC=noticia[2]
        noticia_dict = {
            'id_noticia':noticia[0],
            'titulo': noticia[1],       # Título de la noticia
            'contenido': noticiaC,    # Contenido de la noticia
            'fecha': noticia[3],        # Fecha de la noticia
            'foto': noticia[4],         # Foto asociada a la noticia
            'id_autor': noticia[5]      # ID del autor
        }
        

        noticias_json.append(noticia_dict)
...
def logout():
    ...
    return redirect(url_for('main.main2'))  # Redirigir al login
...
def login():
    ...
    if request.method == 'POST':
        ...
        if result:
            ...
            return redirect(url_for('main.usuario', username=username))  # Redirigir al home si el login es correcto
        else:
...
def nevera():
    ...
    if result:
        ...
        saludo = ""
        empresa= "Desarrollo de tecnologías de la información"
...
def catch_all(path):
    # Página 404 personalizada cuando el usuario accede a una URL que no existe

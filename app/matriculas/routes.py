...
def mtrcls_inscribir(alumno):
    ...
    return render_template('matriculas/inscribir.html', resultados=resultados,alumno =alumno, username=session.get('username'),menu_data =  ast.literal_eval(session['menu']))

@matriculas.route(p+'/lista')
def mtrcls_lista():
    datos,encabezados= listaEstudiantes()
    return render_template('core/tabla2.html', datos= datos, link2=True, encabezados=encabezados ,titulo="estudiantes", username=session.get('username'),menu_data =  ast.literal_eval(session['menu']))

@matriculas.route(p+'/gestion')
def mtrcls_gestion():
    return render_template('matriculas/gestion', username=session.get('username'),menu_data =  ast.literal_eval(session['menu']))

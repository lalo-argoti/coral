...
def check_login():
    ...
    administrador=[]
    #menu_data = [['sesion.login', 'Iniciar sesión']]
    if session.get('rol')==130230414:
        administrador=['coral.databases', 'coral.execute']
    if 'username' not in session :
        session['menu'] = "[['sesion.login', 'Iniciar sesión']]"
    if 'username' not in session and  request.endpoint not in public_routes or (not(session.get('rol')==130230414) and (request.endpoint in administrador)):
        #menu_data = [['sesion.login', 'Iniciar sesión']
